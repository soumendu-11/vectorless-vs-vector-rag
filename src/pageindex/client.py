...
class PageIndexClient:
    ...
    def index(self, file_path: str, mode: str = "auto") -> str:
        ...
        if mode == "pdf" or (mode == "auto" and is_pdf):
            ...
            # Extract per-page text using Docling so queries don't need the original PDF
            from .utils import _get_docling_doc, _docling_page_text, get_number_of_pages
            _doc = _get_docling_doc(file_path)
            _n = get_number_of_pages(file_path)
            pages = [
                {'page': p, 'content': _docling_page_text(_doc, p)}
                for p in range(1, _n + 1)
            ]
